    ...
    def __new__(cls, *args, **kwargs):
        if not hasattr(cls, "_instance"):
            with cls._instance_lock:
                if not hasattr(cls, "_instance"):
                    impl = cls.configure() if hasattr(cls,
                                                      "configure") else cls
                    instance = super(Singleton, cls).__new__(impl, *args,
                                                             **kwargs)
...
class HttpClient(Singleton, BaseHttpClient):
    ...
    def configure(cls):
        ...
        client_cls = {"urllib": UrllibClient,
                      "curl": CurlClient,
                      "requests":
                      RequestsClient}.get(config_client.lower(), None)
        if client_cls:
            return client_cls
...
class WeixinHelper(object):
    ...
    def checkSignature(cls, signature, timestamp, nonce):
        ...
        code = hashlib.sha1(tmp).hexdigest()
    ...
    def getAccessToken(cls):
        ...
        result = HttpClient().get(_ACCESS_URL.format(WxPayConf_pub.APPID,
    ...
    def getUserInfo(cls, access_token, openid, lang="zh_CN"):
        ...
        result = HttpClient().get(_USER_URL.format(access_token, openid, lang))
    ...
    def getAccessTokenByCode(cls, code):
        ...
        result = HttpClient().get(_CODEACCESS_URL.format(
            WxPayConf_pub.APPID, WxPayConf_pub.APPSECRET, code))
        if not isinstance(result, str):
            result = result.decode('utf-8')
        return result
    ...
    def refreshAccessToken(cls, refresh_token):
        ...
        result = HttpClient().get(_REFRESHTOKRN_URL.format(WxPayConf_pub.APPID,
                                                           refresh_token))
        if not isinstance(result, str):
            result = result.decode('utf-8')
        return result
    ...
    def getSnsapiUserInfo(cls, access_token, openid, lang="zh_CN"):
        ...
        result = HttpClient().get(_SNSUSER_URL.format(access_token, openid,
                                                      lang))
        if not isinstance(result, str):
            result = result.decode('utf-8')
        return result
    ...
    def getJsapiTicket(cls, access_token):
        ...
        result = HttpClient().get(_JSAPI_URL.format(access_token))
        if not isinstance(result, str):
            result = result.decode('utf-8')
        return result
    ...
    def jsapiSign(cls, jsapi_ticket, url):
        ...
        signature = '&'.join(['%s=%s' % (key.lower(), sign[key])
                              for key in sorted(sign)])
